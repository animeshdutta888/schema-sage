...
def generate_sql_with_lora(question: str) -> GeneratedSql:
    ...
    if quality_issue:
        ...
        return GeneratedSql(
            sql=fallback.sql,
            explanation=(
                f"LoRA model returned valid SQL, but SchemaSage detected a semantic issue ({quality_issue}), "
                f"so it used the deterministic fallback. {fallback.explanation}"
            ),
            source="rules_fallback",
        )
...
def semantic_quality_issue(question: str, sql: str) -> str | None:
    ...
    if all(term in normalized_question for term in ["compare", "europe", "north america", "revenue"]):
        ...
        if re.search(r"\b(name|customer)\b", select_clause):
    ...
    if "enterprise" in normalized_question and re.search(
        r"\bsegment\s*=\s*['\"]enterprise['\"]",
        sql,
        flags=re.IGNORECASE,
    ):
        if not re.search(r"\bsegment\s*=\s*'enterprise'", sql):
            return "enterprise segment filter must use the demo enum value 'enterprise'"
